...
A `.deck.json` prompt pack describes a multi-panel terminal layout you can clone in one click. v0.1.9 makes loading + authoring easy and includes a setup checker to verify your machine is ready.
...
## Four ways to load a pack

### 1. Bundled packs dropdown (`📚 packs…`)

The simplest path. Click the **📚 packs…** dropdown in the app header. Pick any of the 4 bundled packs. Confirm the replace prompt. Done.
...
Share it on Discord, GitHub, anywhere. Anyone with Pluto's Terminals can drag it in or load via 📁 from file or 🔗 from URL.
...
- **Factory reset** — wipes ALL state (panels, projects, scrollback, API key, theme, welcome flag). Useful for testing or clean reinstall.
...
- Any other process env variable you've set in your shell before launching the app

Resolution order: process env → empty string for unknowns. This is what makes packs cross-machine — pack authors use `${USERPROFILE}/Documents/myproject` instead of hardcoding `C:\Users\someone\...`.
...
All bundled packs use systemPrompt — see `claude-code-basic.deck.json` etc. for examples.
