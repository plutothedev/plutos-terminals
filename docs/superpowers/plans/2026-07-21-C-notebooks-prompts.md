...
- `splitRunnableLines(code, lang) -> { lines } | { notRunnable: reason }` — the line-runnability guard + splitter for C-4's per-line execution. Drops empties and sh-family `#` comments. Returns `notRunnable` when (RE-AUDIT-CORRECTED — presence, NOT count-parity; a well-formed multi-line `if/then/fi` is balanced by definition and parity would wave it through into a stuck PS2 continuation prompt):
  1. any line ends with `\`, or (powershell/pwsh) ends with a backtick `` ` ``;
  2. the code contains `<<` (heredoc);
  3. the block has MORE THAN ONE line (post-drop) AND any line contains a compound keyword or brace — sh-family: word-boundary `if|then|elif|else|fi|for|while|until|do|done|case|esac|function` or `{`/`}`; powershell/pwsh/cmd: `foreach|if|elseif|else|while|for|function|do|switch|try|catch|finally` or `{`/`}`;
  4. (lang-agnostic — sh/pwsh/cmd alike) the block has MORE THAN ONE line AND any line, after trimming trailing whitespace, ends in a binary continuation operator: `&&`, `||`, or a bare `|` (pipe). CARVE-OUT: a lone single trailing `&` (`cmd &` = backgrounding, complete) does NOT flag — match the two-char `&&` and the single `|`, never a solitary `&`. (Trailing binary operators request PS2/`>>` continuation and would park the pane identically to an unbalanced compound.)
  Single-line blocks are NEVER flagged regardless of keywords (a one-line `for i in 1 2 3; do echo $i; done` is complete and runs as one correlated capture). Conservative by design: a multi-line block that merely MENTIONS `if` in a string arg gets flagged — acceptable false-positive; the failure mode it prevents (a live pane stuck at a continuation prompt swallowing input) is far worse.
...
- Execute — AUDIT-CORRECTED (the rev-1 "send as-is" premise was false: without bracketed paste, every embedded newline submits its line as a SEPARATE command with its own OSC PlutoCmd, so a whole-block `runAndCapture` NEVER correlates and every multi-line Run would hang to the 120 s timeout). v1 semantics, implemented in a pure helper `runBlockLines` in `notebookModel.js` (unit-tested with a fake runAndCapture):
  ...
  2. **Line-runnability guard:** `splitRunnableLines` per its C-2 definition (presence-based: multi-line + any compound keyword/brace, trailing `\` or PowerShell backtick, heredoc). Flagged blocks show an explicit "multi-line construct — not runnable in v1" disabled state in the rail. No silent hang, no stuck continuation prompt, ever.
