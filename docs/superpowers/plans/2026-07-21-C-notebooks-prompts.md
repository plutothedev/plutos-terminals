...
- `runnableKind(code, lang) -> "single" | "multiline"` — trivial, complete-by-construction classifier (DESIGN DECISION 2026-07-21, after three audit rounds proved a static "which multi-line blocks are safe to send line-by-line" guard is a shell-lexer-in-a-heuristic with an endless slip-through tail — keywords, then trailing operators, then quotes/parens...). v1 runs ONLY single-line blocks (exactly one line after dropping blanks + sh-family `#` comments). A block with >1 runnable line is `"multiline"` → the rail shows a "multi-line — runs in v1.1" display-only state; it is NEVER sent to the pane, so there is NO continuation-prompt hang surface at all. Multi-line execution is a named v1.1 item with a real design (per-block subshell wrap or a non-interactive runner), not a leaky line-splitter. This deletes the entire keyword/operator/backtick/heredoc guess-work.
...
- Execute — DESIGN-DECISION-CORRECTED (rev-1 "send as-is" was false — each newline submits a separate command; rev-2 tried a static multi-line safety guard, and three audit rounds proved that guard is an endless slip-through game reimplementing a shell lexer). v1 = single-line run-capture only:
  1. `runnableKind(block.code, lang)`. If `"multiline"`, the block is display-only: the rail shows "multi-line — runs in v1.1", the Run button is disabled. Never sent to the pane → zero continuation-hang surface.
