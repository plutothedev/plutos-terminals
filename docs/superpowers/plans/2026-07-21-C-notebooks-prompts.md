...
- `runnableKind(code, lang) -> "single" | "multiline"` — trivial, near-complete-by-construction classifier (DESIGN DECISION 2026-07-21, after three audit rounds proved a static "which multi-line blocks are safe to send line-by-line" guard is a shell-lexer-in-a-heuristic with an endless slip-through tail). v1 runs ONLY single-line blocks. Reduce `code` to runnable lines (drop blanks + sh-family `#` comments). Classify `"multiline"` (display-only, never sent, "runs in v1.1") when EITHER: (a) more than one runnable line remains, OR (b) exactly one line remains but that line is lexically INCOMPLETE. The incompleteness check uses a fixed 1:1 lang→continuation-char table (closed 5-lang enum, NOT a grammar): the line ends with the lang's continuation char — sh/bash → trailing `\`; powershell/pwsh → trailing backtick `` ` ``; cmd → trailing caret `^` (do NOT flag trailing `\` for cmd/powershell — there it is a path separator, e.g. `cd C:\Users\pluto\` is a complete line); OR (lang-agnostic) contains `<<`, or ends with `&&` / `||` / a bare `|` (NOT a lone trailing `&`, which is complete backgrounding). Only a lone, self-contained line is `"single"`. This closes the continuation-hang class for BOTH multi-line blocks and single incomplete fragments. The check is a fixed O(1) suffix/substring set — NOT the keyword/brace enumeration (a one-line `for ...; do ...; done` is self-contained and stays `"single"`). **Accepted, DOCUMENTED residual (auditor's explicit call, matches this plan's hard-quit-gap philosophy):** an unclosed quote or unmatched paren on a single line (`echo "hi`, `(echo x`) is the same incompleteness class but needs char-level balance tracking to detect — deliberately NOT built in v1 (that reopens the lexer); such a block hits the 120 s timeout and the pane may sit at a continuation prompt. Noted in the changelog, not silent.
- Single-line execution reuses `runAndCapture(targetId, theLine)` UNCHANGED (one correlated capture, exactly the AgentMode shape that already works).

- [ ] TDD: tests FIRST — parse (mixed doc: prose, runnable + non-runnable fences, nested backticks in prose, existing output fences, output fence NOT owned when a prose paragraph intervenes), writeOutput (replace, insert-after, idempotence, never-touch-prose pinned by full-document equality, the 256KB slice + marker, timeout exit label), frontmatter roundtrip, CRLF tolerance (parse both, write back preserving the document's dominant line ending), PLUS the audit's adversarial set: (a) two runnable blocks back-to-back with no prose between (block 2 must not be mistaken for block 1's output; `block.index` stays correct after a writeOutput insert shifts lines), (b) an output fence immediately after a NON-runnable fence stays untouched, (c) 4-plus-backtick fences parse or are cleanly ignored (never misparse), (d) literal "```output" mid-paragraph is NOT treated as a fence (scanner is line-anchored), (e) output fence at EOF with no trailing newline. And for `runnableKind`: a lone complete code line → `"single"`; the same after stripping blank + `#`-comment lines → `"single"`; a one-line `for i in 1 2 3; do echo $i; done` → `"single"`; a one-line `cmd &` (backgrounding) → `"single"`. `"multiline"`: any 2+ runnable lines (plain two-command block, `if/then/fi`, `foreach{}`, pipe chain — all just "multiline"), AND the single-incomplete-line cases per the lang table: sh `echo foo\` flagged; **cmd/powershell `cd C:\Users\pluto\` NOT flagged (path separator, stays "single")**; cmd `foo ^` flagged; powershell trailing backtick flagged; lang-agnostic `cat <<EOF`, `echo hi &&`, `ls |` flagged. No keyword-balance cases (none inspected). Do NOT add tests for unclosed-quote/paren single lines — that residual is the documented accepted gap, not a detected case.
...
- Execute — DESIGN-DECISION-CORRECTED (rev-1 "send as-is" was false — each newline submits a separate command; rev-2 tried a static multi-line safety guard, and three audit rounds proved that guard is an endless slip-through game reimplementing a shell lexer). v1 = single-line run-capture only:
  1. `runnableKind(block.code, lang)`. If `"multiline"` (2+ lines OR a single incomplete line), the block is display-only: the rail shows "multi-line — runs in v1.1" (or "incomplete line" for the single-fragment case), the Run button is disabled. Never sent to the pane → the continuation-hang class is closed except the one documented unclosed-quote/paren residual.
  2. If `"single"`, `const r = await runAndCapture(targetId, theLine)` (unchanged ptyBridge; exact correlation, the shape AgentMode already uses).
  Then write the fence via a LATEST-CONTENT REF (`writeOutput(contentRef.current, ...)` — the run can take up to 120 s; closing over the render-time `content` would clobber edits made during the run; mirror RemoteEditor's `textRef`) with `{ output: r?.output ?? "(no output captured)", exit: r?.timedOut ? "timeout" : (r?.exit ?? "?"), timestamp }` + mark dirty. **Run All:** sequential for-of over the `"single"` blocks; stop at first nonzero/timeout exit; multi-line blocks show a visible "skipped (v1.1)" state.
...
- [ ] CHANGELOG Unreleased additions (notebooks + saved prompts bullets, no overclaim — no auto-run, outputs owned-fence-only, prompts per-item sync, **v1 runs single-line blocks only (multi-line = v1.1)**, and the documented residual: a single line with an unclosed quote/paren can time out / park the pane; the hard-quit autosave gap).
