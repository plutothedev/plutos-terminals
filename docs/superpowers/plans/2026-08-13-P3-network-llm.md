...
2. **Cancel is a DELIVERABLE, not a verification (audit H1 — no contract exists on either side):** caller-minted request id → `llm_stream_cancel(id)` sets the token via **insert-or-set-true (entry API — a cancel can land BEFORE the stream registers; set-if-present would no-op and the stream runs to max_tokens)**; llm_stream `entry(id).or_default()`, pre-checks the flag before firing the request, checks each SSE-loop iteration, and **removes the entry on EVERY exit (Ok/Err/cancelled) via a drop guard** so the map can't grow one entry per call for app lifetime. Dropping `resp` closes the connection (pool-safe on the shared client — partially-consumed connections are reset, not reused). llmStream returns `{promise, cancel}`.
...
2. sync_git.rs: all 3 command bodies → spawn_blocking (git2 values created inside the closure — Send verified); `maybe_gc_at` → detached task after push returns, **serialized by a `static` tokio Mutex acquired via `blocking_lock()` as the FIRST statement INSIDE each spawn_blocking closure (all 4 sites — 3 commands + gc; legal on the blocking pool, never call from async context; the guard-across-await alternative releases early if the outer future drops)** (audit H2: unserialized, gc's dir-swap on POSIX succeeds under open handles and a concurrent push can commit the OLD workdir — silent settings revert; Windows rename just fails benignly). App-exit mid-gc already bounded (pre-clean + re-clone paths audit-verified).
