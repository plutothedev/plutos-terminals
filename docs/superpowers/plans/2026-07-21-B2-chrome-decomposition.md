...
Props: `dockTab`, `setDockTab`, **and `collapseDock`** (the "Collapse panel" button at `:1138` calls it — plan-audit catch; note `collapseDock` is NOT identity-stable, see the useDockResize note in the foundation facts, pass it anyway, no new stabilization). `dockTab` state STAYS in TerminalsTab (it gates `useSystemStats` at `:252` and the dock body at `:1141-1154`). Icons `SFolder, SAsk, SPulse` imported directly in the new file. Recipe rule 1 applies: enumerate the block's free identifiers by reading before finalizing.
