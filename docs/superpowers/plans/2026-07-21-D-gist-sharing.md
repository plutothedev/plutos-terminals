...
- `gist_create(filename, content, public: bool) -> { id, url, htmlUrl }` — resolves the token (gh first, then PAT), assembles the `POST https://api.github.com/gists` body `{ description, public, files: { <filename>: { content } } }` where **`description` is a FIXED CONSTANT string `"Shared via Pluto's Terminals"` — never parameterized, never derived from content/filename/title (a content-derived description is the same leak class as the filename was)**. Sends via a reqwest client (mirror `llm.rs`: `Client::builder().timeout(30s).user_agent("plutos-terminals").build()`, `.bearer_auth(token)`, `Accept: application/vnd.github+json`), status-checks via a local `http_error`-style helper before parsing, returns `{ id, url (the api url), htmlUrl (html_url) }`. On no auth → `Err("no GitHub token")`.
...
- `buildShare(kind, rawText, dateStamp) -> { filename, masked, hits }` — `hits = scanSecrets(rawText)`, `masked = maskSecrets(rawText, hits)`. **AUDIT-CRITICAL: the filename is CONTENT-INDEPENDENT — it is NEVER derived from the command/title/rawText, because the gist filename is part of the same POST body and a raw title like `curl -H "Authorization: Bearer sk-..."` would leak the secret in the filename even when the content is masked.** Filename = the spec's fixed scheme: `plutos-terminal-share-${dateStamp}.${kind === "transcript" ? "md" : "txt"}`. **`dateStamp` is an INJECTED parameter (3rd arg), NOT computed inside this pure module** (no `new Date()` here — that would break determinism and the filename-independence test; the caller stamps it, mirroring the injected-`now` pattern at `commands.rs:804-819`). Return ONLY `{ filename, masked, hits }` — `raw` is NOT returned. The modal shows `masked` and uploads `masked`.
...
- **Transcript share:** tab menu item computes the transcript name (`transcriptName(projectName, tabId)` pattern — `<project-or-tab>-<last6>`), `invoke("transcript_read_all", { name })`, opens ShareModal with `{ kind:"transcript", title:<tab label>, rawText, dateStamp }` (same `todayDate()` source as the block handler). Empty transcript → toast "nothing recorded yet", no modal.
