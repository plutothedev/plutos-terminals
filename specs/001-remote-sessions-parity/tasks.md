<!-- (C) -->
---
description: "Task list for Unified Remote Sessions — first-class RDP & VNC"
---

# Tasks: Unified Remote Sessions — First-Class RDP & VNC

**Input**: Design documents from `specs/001-remote-sessions-parity/`

**Prerequisites**: plan.md, spec.md, research.md, data-model.md, contracts/ui-contracts.md, quickstart.md

**Tests**: No automated UI test harness exists in-repo; verification is **manual / live** per
Constitution Principle II. Test tasks are therefore live-verification tasks (screenshot the
running app), not unit tests.

**Organization**: Grouped by user story (P1 → P2 → P3) for independent, incremental delivery.

## Format: `[ID] [P?] [Story] Description`

- **[P]**: Can run in parallel (different files, no incomplete dependencies)
- **[Story]**: US1 / US2 / US3
- All paths are under `src/features/terminals/` unless noted.

---

## Phase 1: Setup

**Purpose**: Confirm a clean baseline before touching code.

- [ ] T001 Launch the app on branch `001-remote-sessions-parity` (`npm run tauri dev`), confirm it renders, and screenshot the running window as the pre-change baseline (Principle II).

---

## Phase 2: Foundational (Blocking Prerequisites)

**Purpose**: Shared state/launch plumbing every user story depends on.

**⚠️ CRITICAL**: Complete before any user story.

- [X] T002 Add an in-memory per-session password cache with helpers `rememberSessionPassword(sessionId, secret)`, `getSessionPassword(sessionId)`, `forgetSessionPassword(sessionId)` in `TerminalsTab.jsx` (app-run lifetime, never persisted) — contract C3, FR-008.
- [X] T003 Add a focus-existing-tab lookup (`focusExistingProjectTab`) used by `openProjectInPanel` in `TerminalsTab.jsx` — contract C2 step 1, FR-012. (Scoped to the RDP/VNC launch branches, where duplicate connections are costly; local/SSH keep multi-tab behavior.)

**Checkpoint**: Shared launch/credential plumbing ready.

---

## Phase 3: User Story 1 — Save and reconnect a remote desktop (Priority: P1) 🎯 MVP

**Goal**: Create, save, and one-click-reconnect RDP/VNC sessions from the sidebar; they persist across restart.

**Independent Test**: Add a VNC and an RDP session, save, restart the app, click each → tab opens and begins connecting; sessions still listed.

- [X] T004 [P] [US1] Add `rdp` and `vnc` type tabs to the type selector, their form fields (RDP: host/port[3389]/username/domain; VNC: host/port[5900]), `canSave` rules (name + host), and `onSave` payloads in `ProjectDialog.jsx` — contract C1, FR-001/FR-002.
- [X] T005 [US1] Extend `openProjectInPanel` in `TerminalsTab.jsx` with `type === "vnc"` and `type === "rdp"` branches: build the tab (`vnc`/`rdp` field + `projectId`), resolve the password (session cache → else prompt), abort cleanly on cancel, then `spawnSessionTab` — contract C2, FR-004 (depends on T002, T003).
- [X] T006 [US1] Wire the saved-session password prompt for rdp/vnc launch by reusing `RdpConnectModal`/`VncConnectModal` pre-filled + locked on the saved host/port (`launchVnc`/`launchRdp`), feeding the secret into the session cache + tab bridge in `TerminalsTab.jsx` (depends on T005).
- [X] T007 [P] [US1] Add `rdp`/`vnc` type→icon mapping in the sidebar rows (`osIconFor` already maps rdp/vnc → Windows icon) in `ProjectSidebar.jsx` — contract C7, FR-007.
- [X] T008 [P] [US1] Add `rdp`/`vnc` type→icon mapping (RDP 🪟, VNC 🖱) in the saved-session grid in `MobaHomeScreen.jsx` — contract C4.
- [X] T009 [US1] Verify live (Principle II): create+save VNC and RDP confirmed by user (4 type tabs, icons, locked password prompt on launch); persistence confirmed via on-disk state round-trip. ✅

**Checkpoint**: RDP/VNC are saveable + one-click launchable + durable — MVP usable.

---

## Phase 4: User Story 2 — Organize and edit saved remote desktops (Priority: P2)

**Goal**: Full lifecycle parity — edit, rename, color, folder, delete, unified search, forget-password.

**Independent Test**: Edit a saved RDP/VNC session's details, color, folder; search across types; delete; confirm persistence.

- [X] T010 [US2] Populate rdp/vnc fields in `ProjectDialog` edit mode (init state from `initial.rdp`/`initial.vnc` in the open effect; `sessionType()` already returns stored `type`) in `ProjectDialog.jsx` — FR-005.
- [X] T011 [P] [US2] Include rdp/vnc `host` in the unified sidebar search filter in `ProjectSidebar.jsx` so SSH/RDP/VNC match together — FR-007.
- [X] T012 [P] [US2] Wire the "forget password" context-menu action for rdp/vnc sessions to `forgetSessionPassword` in `ProjectSidebar.jsx` + `TerminalsTab.jsx` — FR-008.
- [ ] T013 [US2] Verify live: edit host/port, set color+folder, search across all three types, delete a session; relaunch → changes persisted; screenshot quickstart US2 (depends on T010–T012). **PARTIALLY live-verified (2026-05-29) via Peekaboo; not yet fully passed.**
  - ✅ Created a VNC session live (host 10.0.0.50, port 5900 default) → appeared in sidebar with VNC/Windows icon.
  - ✅ Edit-session dialog opens with rdp/vnc fields **pre-populated** (HOST/PORT/NAME/FOLDER) — confirms T010 in edit mode.
  - ✅ Assigned the VNC to the "QA Lab" folder live (sidebar reflowed; folder badge 1→2; FOLDER field showed "QA Lab" in the edit dialog).
  - ✅ Deleted the VNC live (folder badge 2→1; row gone).
  - ✅ Context menu exposes COLOR picker (8 swatches + clear), FOLDER move, **Forget saved password** (T012), **Remove**. Color persistence independently evidenced by the pre-existing RDP-QA session showing a persisted blue color dot inside QA LAB.
  - ✅ Edit dialog also verified on the **RDP** session RDP-QA: opened pre-populated with HOST `10.0.0.99`, PORT `3390`, USERNAME `Administrator`, NAME `RDP-QA`, FOLDER `QA Lab`, with Save/Cancel (RDP form = host/port/username/domain, FR-001).
  - ✅ Launch flow (quickstart US1 #5 + FR-008): clicking the saved RDP session opened the **RDP connect modal** "Connect — RDP-QA", pre-filled host/port/username, prompting only for password, with the text "The password is used only for this connection and is never saved" and "Connects with NLA (CredSSP)…" — dark-themed.
  - ✅ **Relaunch-persistence (2026-05-29, session 2):** after TWO full quit→relaunch cycles (clean SIGTERM + `npm run tauri dev`), the saved sessions persist intact — `QA LAB ▸ RDP-QA` (type rdp, color blue, folder "QA Lab", port 3390) + `Hermes` (ssh, `root@45.33.11.112:22`). Confirmed both visually (window capture `/tmp/pluto_verify/A_relaunch4.png` + `A2_relaunch_persist2.png`) and via the on-disk WebKit sqlite (`terminalsState.projects`). RDP-QA's tab also auto-restores and re-attempts its saved `10.0.0.99:3390` → inline error + **Retry** (FR-009, live again).
  - ⚠️ STILL harness-blocked (NOT app defects) — need a human at the physical keyboard: host/port **change-and-save** round-trip (typing the new PORT) and live **search filtering** (typing in the search box). **Re-confirmed this session** that BOTH AX `set_value` AND synthetic `type` fail to drive these React-controlled inputs: `set_value` reports success but doesn't fire React's `onChange` (list never filters; field keeps placeholder), and synthesized CGEvents don't reach the WKWebView at all.
  - Harness note (updated 2026-05-29 session 2): this Tauri WKWebView accepts AX **button presses** (`click`/AXPress on `elem_*` open the Add-Session dialog + switch type tabs reliably) but NOT text entry (set_value no-ops on React state; synthetic typing doesn't land). **`mcp__peekaboo__inspect_ui` works** (pure AX, no ScreenCaptureKit) for reading the tree; **`mcp__peekaboo__see`/`image` time out** (SCK stalls) — capture instead with **`screencapture -l<windowID> -o -x`** (reads the WindowServer backing store; survives occlusion; doesn't auto-close modals). A relaunched window paints black until moved to the **active Space** (`space move-window --to_current`) — WKWebView throttles render off-Space.

**Checkpoint**: RDP/VNC have full SSH-parity lifecycle.

---

## Phase 5: User Story 3 — Verify the remote-session story works live (Priority: P3)

**Goal**: Robust failure UX + quick-connect promotion, then end-to-end live verification.

**Independent Test**: Drive SSH against the VPS end-to-end; drive RDP/VNC UI flow incl. error/retry, cancel, reconnect, focus-existing.

- [X] T014 [P] [US3] Add an inline error state + **Retry** button to `VncView.jsx` (transition on `vnc-exit`/connect failure; Retry re-invokes connect via `retryKey`) — contract C6, FR-009.
- [X] T015 [P] [US3] Add an inline error state + **Retry** button to `RdpView.jsx` (transition on `rdp-exit`/connect failure; Retry re-invokes connect via `retryKey`) — contract C6, FR-009.
- [X] T016 [P] [US3] Add a "Save this connection" affordance + `onSaveSession`/`initial`/`lockConnection` to `VncConnectModal.jsx` — contract C5, FR-013.
- [X] T017 [P] [US3] Add a "Save this connection" affordance + `onSaveSession`/`initial`/`lockConnection` to `RdpConnectModal.jsx` — contract C5, FR-013.
- [X] T018 [US3] Wire `onSaveSession` (`saveQuickConnection`) from both connect modals to append a saved session (no password) to `state.projects` in `TerminalsTab.jsx` — FR-013 (depends on T016, T017).
- [X] T019 [US3] Live SSH verification vs the VPS (45.33.11.112, OpenSSH 9.6): connect + interactive shell confirmed (`ls` returned home dir). Uncovered + fixed a real bug — initial MOTD/prompt was lost to a listener race (`fix(ssh)` 4c396a7); connect is ~700ms. SC-004 connect ✅. (SFTP + reconnect spot-checks optional follow-up.)
- [X] T020 [US3] RDP/VNC UI-flow verification: user confirmed inline error + **Retry** on dead host (FR-009), **focus-existing** on re-click (FR-012), and **"Save this connection"** from quick-connect (FR-013). ✅
- [X] T021 [US3] Credential safety check — FR-008, SC-005. VERIFIED by decoding the app's on-disk WebKit localStorage (`~/Library/WebKit/plutos-terminals/.../localstorage.sqlite3`): zero password/secret tokens in any value; saved records carry no password field. Secrets live only in-memory (`ptyBridge` + `sessionPwRef`). Persistence confirmed (state round-trips to disk; cleared `projects:[]` persisted).
- [X] T022 [US3] Record the **[DEFERRED]** live RDP/VNC connection tests (US3 #5) as outstanding — captured in spec.md (US3 #5 `[DEFERRED]`) and quickstart.md (Deferred section). Not marked complete by proxy.

**Checkpoint**: Story verified live where servers exist; deferred items explicitly tracked.

---

## Phase 6: Polish & Cross-Cutting Concerns

- [X] T023 [P] Ensure the `(C)` marker is present on every created/modified file in `src/features/terminals/` — Principle III. (Added to TerminalsTab/ProjectSidebar/ProjectDialog; siblings already had it.)
- [ ] T024 [P] MobaXterm v12.4 **dark** fidelity pass on the new RDP/VNC dialog tabs + modal affordances; side-by-side screenshot compare — FR-010, SC-006. (New UI reuses existing `--phn-*` dark tokens.) **PARTIALLY observed live (2026-05-29):** the ADD/EDIT SESSION dialogs render all four type tabs (🖥 Local / 🌐 SSH / 🪟 RDP / 🖱 VNC) with dark panels + blue accent on the active tab + monospace labels; the **RDP connect modal** ("Connect — RDP-QA") renders dark-themed and pre-filled.
  - ✅ (2026-05-29, session 2) Captured the **ADD SESSION dialog on the VNC tab** dark-themed (`/tmp/pluto_verify/T024_dialog_vnc.png`): the four type tabs (🖥 Local / 🌐 SSH / 🪟 RDP / 🖱 VNC) with VNC active + blue accent, **HOST** + **PORT `5900`** (VNC default, FR-002), NAME, FOLDER (+helper "Groups this session under a collapsible folder…"), TAGS (+helper "Filter the sidebar by a tag with #tag"), on the dark dialog surface with a dimmed backdrop and Cancel/Add (Add disabled until name+host). Strengthens the dialog-tab fidelity portion.
  - ✅ (2026-05-29, session 2) **Structural skin comparison done** vs a user-supplied MobaXterm reference (Personal Edition **v24.1, light** chrome — main window). Strong layout fidelity: identical IA (menu bar → labeled-icon toolbar → quick-connect bar → [sessions tree │ black tabbed terminal] → status bar), flat icon+label toolbar (MultiExec→MultiX, Tunneling→Tunnel), quick-connect bar, collapsible sessions tree, chrome-style tabs + "+", and terminal stays black in both chromes. Captured Pluto in both skins: `T024_pluto_light_main.png` (light, like-for-like vs the ref) + `A_relaunch4.png` (dark). Divergences (by design): persistent right SFTP/Assistant/Monitor dock (MobaXterm = left slide-out), a bottom F-key hint bar (MobaXterm has none), and Pluto's tab-strip stays dark in light chrome.
  - **DARK side-by-side DONE (2026-05-29)** vs a user-supplied MobaXterm **Professional v20.6 dark** reference (main window). **Structural fidelity: strong** — same information architecture (menu bar → labeled-icon toolbar → sessions/file tree → chrome-style tabs → system status bar), pure-black terminal, and a segmented/colored prompt. **Two real color divergences from MobaXterm's dark theme**, both apparently deliberate (the skin calls itself "Dark charcoal chrome") but worth noting against the spec's "match MobaXterm" bar:
    1. Chrome surface `--phn-surface-bg: #181818` (near-black) vs MobaXterm's medium grey ≈`#2d2d2d`–`#383838` → **Pluto reads darker**.
    2. Accent `--phn-link: #4DAAFC` (**blue**) vs MobaXterm's **teal** ≈`#4aa8c0`.
    Terminal ANSI palette matches closely (cyan `#5fd7d7` / green `#5fd75f` / yellow `#e5e510` / red `#ff5f5f`). Layout difference: Pluto docks SFTP/Assistant/Monitor on the **right**; MobaXterm uses a **left** vertical tab. **Decision left to the user:** accept the charcoal/blue deviation, or tune the two tokens toward MobaXterm grey/teal. Box left unchecked pending that call (Principle II — "close enough" is the user's to judge).
  - ⚠️ Still open: the **VNC connect modal** (VncConnectModal) capture — needs a saved VNC, whose creation requires typing (harness-blocked). Neither MobaXterm reference showed its session-creation dialog, so Pluto's tabbed Local/SSH/RDP/VNC selector is compared at the token level only (a deliberate modernization of MobaXterm's session-type icon grid, not a pixel clone).
- [ ] T025 Run the full `quickstart.md` end-to-end and capture the screenshot set as the completion evidence — Principle II. **PENDING live run.**

> **Implementation status (2026-05-29):** All code tasks complete; frontend `npm run build` green. No backend changes. The remaining unchecked tasks (T001, T009, T013, T019–T021, T024, T025) are **live-verification gates** that require running the app (and, for T019, the VPS) — they are intentionally NOT checked off until observed in the running app per Principle II.

---

## Dependencies & Execution Order

- **Phase 1 (Setup)** → **Phase 2 (Foundational)** blocks all stories.
- **US1 (P3)** depends on Foundational (T002, T003). It is the MVP.
- **US2 (P4)** depends on Foundational; builds on US1's dialog but is independently testable.
- **US3 (P5)** depends on Foundational; T014–T018 are feature work, T019–T022 are verification.
- **Polish (P6)** after the desired stories.

### Within stories

- US1: T004 (dialog) ∥ T007/T008 (icons) can run parallel; T005 needs T002/T003; T006 needs T005; T009 verifies after all.
- US3: T014/T015/T016/T017 are parallel (different files); T018 needs T016/T017.

## Parallel Opportunities

- US1: `[P]` T004, T007, T008 together (ProjectDialog, ProjectSidebar, MobaHomeScreen — distinct files).
- US3: `[P]` T014, T015, T016, T017 together (VncView, RdpView, VncConnectModal, RdpConnectModal — distinct files).
- Polish: `[P]` T023, T024 together.

## Implementation Strategy

### MVP first (US1 only)
1. Phase 1 Setup → 2. Phase 2 Foundational → 3. Phase 3 US1 → **STOP & VALIDATE** (T009) → demo.

### Incremental delivery
US1 (saveable+launchable+durable) → US2 (organize/edit) → US3 (robust failure UX + live verify). Each adds value without breaking the prior.

## Notes

- `[P]` = different files, no incomplete deps. Commit after each task or logical group.
- No backend (`src-tauri/`) changes expected — transports already exist.
- Live verification tasks are gates, not formalities (Principle II). Green build ≠ done.
- The deferred live RDP/VNC connection test stays outstanding until test servers exist (T022).

**Total tasks**: 25 — Setup 1, Foundational 2, US1 6, US2 4, US3 9, Polish 3.
